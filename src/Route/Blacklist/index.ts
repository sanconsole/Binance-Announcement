...
class Route {
  public path: SchemaNames = "blacklist";
  public router = express.Router();
  public schema = BlacklistSchema;
  ...
  private initializeRoutes() {
    ...
    this.router.get(`/getTotal/${this.path}/`, this.getTotalBlacklist);
  ...
  private getTotalBlacklist = async (
    request: Request,
    response: Response,
    next: NextFunction
  ) => {
  ...
  private add = async (
    ...
    try {
      const blacklistRes = new this.schema(request.body)
      const data = await blacklistRes.save()
      response.status(200).json({
          status: true,
          data: data,
      });
